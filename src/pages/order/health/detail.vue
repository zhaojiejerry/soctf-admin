<template>
  <div class="hb-health-detail">
    <wx-header v-show="!isPlatform" showBack>
      <span slot="headerTitle">体检详情</span>
      <span class="second-font" slot="headerOther">(业务订单编号&nbsp;{{ healthId }})</span>
    </wx-header>
    <div
        ...
        >
          <el-col :span="colNumLeft" class="col-container" :class="{isPlatformPart: isPlatform}">
            <div class="bg-part">
              ...
              <div class="detail-user-info" v-if="!isPlatform">
                <div class="left">
                  <p>
                    <label>用户姓名：</label>
          ...
          <el-col :span="colNumRight" class="col-container">
            <div class="bg-part" style="padding-right: 10px">
              ...
              <div v-if="healthData.checkStatus == 0 " class="uncheck-info no-pay-parent">
                <div class="no-pay">
                ...
                <div class="update-wrap">
                  ...
                  <div class="result-btn" :class="{isPlatformResultBtn: isPlatform}">
                    <el-button
                      ...
                      v-if="!openUpdate && healthData.qualifiedStatus == 1 && btnPermissions.indexOf('1000411') != -1"
                    >修改</el-button>
                    <el-button
                      size="mini"
  ...
  methods: {
      ...
      if (this.height < 155) {
        for (let i = 0; i < licenseTypeArray.length; i++) {
          if (bigCar.indexOf(licenseTypeArray[i]) != -1) {
            this.$message.error('大型车身高需高于155CM')
            return
          }
        }
      }
    ...
    rejectReport () {
      this.$prompt('', '请填写驳回理由', {
        ...
        inputType: 'textarea',
        inputPattern: /^.{1,25}$/,
        inputErrorMessage: '理由不能为空且最多25个字',
        inputPlaceholder: '注意：驳回理由不能为空',
        confirmButtonClass: 'messageBoxButton',
        customClass: 'reject-reason-prompt',
        message: '<div style="font-size:12px; color:#fd7715"><i class="el-icon-warning" style="margin-right:6px;"></i>驳回后无法撤销且无法退款，请确保已仔细审核所有体检信息</div>',
