        ...
        <el-table ref="subAccountListTable" :header-cell-style="{background:'#f7f7f7', color:'#333333', fontWeight: 'bold'}" :cell-style="{fontSize: '12px'}" :data="rankingList" class="list-table" tooltip-effect="dark">
          <el-table-column align="center" label="排行">
            <template slot-scope="scope">
              <div class="order">
                <img v-if="(scope.$index+1)+(currentPage-1)*pageSize == 1" src="@/assets/images/medal1.png" alt="">
                <img v-else-if="(scope.$index+1)+(currentPage-1)*pageSize == 2" src="@/assets/images/medal2.png" alt="">
                <img v-else-if="(scope.$index+1)+(currentPage-1)*pageSize == 3" src="@/assets/images/medal3.png" alt="">
                <span v-else>{{ (scope.$index+1)+(currentPage-1)*pageSize }}</span>
              </div>
            </template>
          </el-table-column>
